...
| Function | Purpose |
|---|---|
| `set_mirror()` | Configure CRAN / Bioconductor download mirrors |
| `pkg_functions()` | List exported functions from any installed package |

For installation, updates, and status checks, we recommend
[pak](https://pak.r-lib.org/) — it provides a more elegant and performant
workflow with parallel downloads and a unified interface for CRAN, GitHub,
and Bioconductor.
...
respected by R's native install functions and by pak. The previous settings
are returned invisibly, making it easy to restore them.
...
performant package management workflow. It supports parallel installation,
automatic dependency resolution, and a unified interface for all package
sources — which is why `inst_pkg()`, `check_pkg()`, `update_pkg()`, and
`pkg_version()` were removed from evanverse.
