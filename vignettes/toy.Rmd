...
`toy_gene_ref()` returns a small deterministic reference table compatible with
`gene2entrez()` / `gene2ensembl()` workflows. The human reference includes the
symbols used by `toy_gmt()`, so GMT parsing and gene ID conversion examples can
run offline together.
