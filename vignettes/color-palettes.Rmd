...
blues_5 <- get_palette("blues", type = "sequential", n = 5)
cat("\nBlues palette (5 colors):\n")
print(blues_5)

# Get all available colors from a palette
blues_all <- get_palette("blues", type = "sequential", n = "all")
...
preview_palette("oranges", type = "sequential")
title("Sequential: Oranges", cex.main = 1.2, col.main = "#0D47A1")

# Diverging palette preview
preview_palette("redblue", type = "diverging")
title("Diverging: Red-Blue", cex.main = 1.2, col.main = "#0D47A1")
...
seq_colors <- get_palette("blues", type = "sequential", n = 9)
...
p2 <- ggplot(cor_long, aes(x = X, y = Y, fill = Correlation)) +
  ...
  labs(
    ...
    subtitle = "Using blues palette for continuous correlation data",
    x = "Variables",
    y = "Variables"
  ) +
  theme_minimal() +
...
div_colors <- get_palette("redblue", type = "diverging", n = 11)
...
cat("  • Diverging: Use 'redgreen' or 'redblue' for fold changes\n\n")
...
- 📊 [Visualization Showcase](visualization-showcase.html) - Advanced plotting techniques
