...
receive the scalar `default` (NA by default). Set `name` to write to a new
column instead of overwriting the source. Explicit `NA` values in `dict` are
kept as matched values rather than replaced by `default`.
...
If the reference table contains duplicated symbols after case normalisation,
the first match is used and a warning is emitted.
...
> removed from the result. If every line is malformed, both functions raise an
> error because no valid gene set can be returned.
...
Calculates the number of ordered arrangements of `k` items from `n` distinct
items:
