...
compile_palettes <- function(palettes_dir = "inst/extdata/palettes",
  ...
  if (!requireNamespace("cli", quietly = TRUE)) stop("cli package required.")
  ...
  for (json_file in json_files) {
    ...
    if (name %in% names(palettes[[type]])) {
      cli::cli_alert_warning("⚠️ Duplicate palette '{name}' in '{type}', will overwrite.")
  ...
  tryCatch({
    ...
    log_lines <- c(log_lines, sprintf("[%s] Error: %s", Sys.time(), msg))
  })
