...
| Area                  | Functions                                                                                                                                                                                                                                                                                                |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Data frame utilities  | [`df2list()`](https://evanbio.github.io/evanverse/reference/df2list.md), [`df2vect()`](https://evanbio.github.io/evanverse/reference/df2vect.md), [`recode_column()`](https://evanbio.github.io/evanverse/reference/recode_column.md), [`view()`](https://evanbio.github.io/evanverse/reference/view.md) |
...
Groups one column’s values by another column and returns a named list.
Useful for building marker lists, gene set inputs, or any grouping
operation that downstream functions expect as a list.
...
Extracts two columns and returns a named vector, using one column as
names and the other as values. The original value type is preserved.
...
The name column must not contain `NA`, empty strings, or duplicates —
all three are caught at input and raise an informative error.
...
Returns an interactive `reactable` widget with search, filtering,
sorting, and pagination. In RStudio the widget renders in the Viewer
pane; in other environments it renders in the default HTML output.
...
requires the `reactable` package. If it is not installed, the function
...
Columns: `file`, `size_MB`, `modified_time`, `path`.
...
Returns the same four-column data frame as
[`file_ls()`](https://evanbio.github.io/evanverse/reference/file_ls.md)
but for an explicit vector of file paths rather than a directory scan.
...
Duplicate paths in the input are silently deduplicated. Missing files
raise an error listing all unresolved paths.

------------------------------------------------------------------------

### `file_tree()` — Print a directory tree

Prints the directory structure in tree format. Returns the lines
invisibly so output can be captured if needed.
...
frame: the original input (`symbol`), the case-normalised form used for
matching (`symbol_std`), and the converted ID.
...
| Source                                                                                      | When to use                                          |
|---------------------------------------------------------------------------------------------|------------------------------------------------------|
| [`toy_gene_ref()`](https://evanbio.github.io/evanverse/reference/toy_gene_ref.md)           | Examples, tests, offline work — 20 genes, no network |
| [`download_gene_ref()`](https://evanbio.github.io/evanverse/reference/download_gene_ref.md) | Production analysis — full genome via biomaRt        |
...
|-----------|------------------------------------------------------------------------------------------------|
| `"human"` | [`toupper()`](https://rdrr.io/r/base/chartr.html) — `"tp53"` and `"TP53"` both match `TP53`    |
...
collections such as MSigDB. Each line encodes one gene set: `term`,
`description`, and a tab-separated list of gene symbols.

[`toy_gmt()`](https://evanbio.github.io/evanverse/reference/toy_gmt.md)
writes a minimal GMT file to a temp path for offline use:
...
a named list of ID vectors ready for enrichment analysis.
